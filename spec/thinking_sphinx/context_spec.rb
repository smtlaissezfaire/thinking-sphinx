...
describe ThinkingSphinx::Context do
  ...
  describe '#prepare' do
    ...
    it "should not load the models if the list of models is set" do
      ThinkingSphinx::Context.preload_models = []
      @class_name.should_not_receive(:constantize)
      @context.prepare
    end

    class ARClass; end

    it "should load a model given as a string in the preloaded models" do
      class_name = "ARClass"

      ThinkingSphinx::Context.preload_models = [class_name]
      class_name.should_receive(:constantize).and_return true
      @context.prepare
    end

    it "should load a model given as a symbol in the preloaded models" do
      # can't stub methods on symbol
      class_name_as_string = "ARClass"
      class_name_as_sym = mock :ARClass, :to_s => class_name_as_string

      ThinkingSphinx::Context.preload_models = [class_name_as_sym]
      class_name_as_string.should_receive(:constantize).and_return true
      @context.prepare
    end
  end
